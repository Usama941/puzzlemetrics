...
export function ProductTable({ initial }: Props) {
  ...
  async function togglePublished(row: Product) {
    try {
      setPending(row.id);
      const next = { ...row, published: !row.published };
      await adminJson(`/api/admin/products/${row.id}`, {
        method: "PUT",
        body: JSON.stringify({
          badge: next.badge,
          label: next.label,
          ...
          stats: next.stats,
          primaryCtaText: next.primaryCtaText,
          primaryCtaUrl: next.primaryCtaUrl,
          secondaryCtaText: next.secondaryCtaText,
          secondaryCtaUrl: next.secondaryCtaUrl,
          theme: next.theme,
          mockup: next.mockup,
          mockupSide: next.mockupSide,
          order: next.order,
  ...
  async function confirmDelete() {
    if (!deleteId) return;
    try {
      await adminJson(`/api/admin/products/${deleteId}`, { method: "DELETE" });
        ...
        <table className="w-full min-w-[720px] text-left text-sm">
          <thead>
            <tr className="border-b border-white/10 text-white/45">
              <th className="w-[60px] px-3 py-3 font-medium">Theme</th>
              ...
              return (
                <tr key={row.id} className="border-b border-white/[0.06]">
                  <td className="px-3 py-3">
                    <div
                      className="h-11 w-11 shrink-0 overflow-hidden rounded-[10px] border border-white/10"
                      style={{ background: t.gradient }}
                      title={row.theme ?? "violet"}
                    />
                  </td>
