...
export function TestimonialTable({ initial }: Props) {
  ...
  async function patch(row: Testimonial, patch: Partial<Testimonial>) {
    try {
      setPending(row.id);
      const next = { ...row, ...patch };
      await adminJson(`/api/admin/testimonials/${row.id}`, { method: "PUT", body: JSON.stringify(next) });
  ...
  async function confirmDelete() {
    if (!deleteId) return;
    try {
      await adminJson(`/api/admin/testimonials/${deleteId}`, { method: "DELETE" });
          ...
          <tbody className="text-white/85">
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-white/[0.06]">
                <td className="px-4 py-3 font-medium text-white">{row.author}</td>
                <td className="px-4 py-3">{row.platformLabel}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`/admin/testimonials/${row.id}/edit`} className="mr-2 inline-flex rounded-lg p-2 text-[#7B6EE8] hover:bg-white/5">
