...
export function PortfolioTable({ initial }: Props) {
  ...
  async function patch(row: PortfolioProject, patch: Partial<PortfolioProject>) {
    try {
      setPending(row.id);
      const next = { ...row, ...patch };
      await adminJson(`/api/admin/portfolio/${row.id}`, {
        method: "PUT",
  ...
  async function confirmDelete() {
    if (!deleteId) return;
    try {
      await adminJson(`/api/admin/portfolio/${deleteId}`, { method: "DELETE" });
            ...
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-white/[0.06]">
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`/admin/portfolio/${row.id}/edit`} className="mr-2 inline-flex rounded-lg p-2 text-[#7B6EE8] hover:bg-white/5">
