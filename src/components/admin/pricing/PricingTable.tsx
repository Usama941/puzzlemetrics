...
type Props = { initial: PricingPlan[] };

export function PricingTable({ initial }: Props) {
  ...
  async function patch(row: PricingPlan, patch: Partial<PricingPlan>) {
    try {
      setPending(row.id);
      const next = { ...row, ...patch };
      await adminJson(`/api/admin/pricing/${row.id}`, { method: "PUT", body: JSON.stringify(next) });
  ...
  async function confirmDelete() {
    if (!deleteId) return;
    try {
      await adminJson(`/api/admin/pricing/${deleteId}`, { method: "DELETE" });
              ...
              <tr key={row.id} className="border-b border-white/[0.06]">
                ...
                <td className="px-4 py-3">
                  <ToggleSwitch
                    ...
                    onChange={() => patch(row, { highlighted: !row.highlighted })}
                  />
                </td>
                <td className="px-4 py-3">
                  <ToggleSwitch
                ...
                <td className="px-4 py-3 text-right">
                  <Link href={`/admin/pricing/${row.id}/edit`} className="mr-2 inline-flex rounded-lg p-2 text-[#7B6EE8] hover:bg-white/5">
