...
export function ServiceTable({ initial }: Props) {
  ...
  async function toggle(row: Service, key: "published") {
    try {
      setPending(row.id);
      const next = { ...row, [key]: !row[key] };
      await adminJson(`/api/admin/services/${row.id}`, {
        method: "PUT",
  ...
  async function confirmDelete() {
    if (!deleteId) return;
    try {
      await adminJson(`/api/admin/services/${deleteId}`, { method: "DELETE" });
              ...
              <tr key={row.id} className="border-b border-white/[0.06]">
                ...
                <td className="px-4 py-3">
                  <ToggleSwitch
                    ...
                    onChange={() => toggle(row, "published")}
                  />
                </td>
                <td className="px-4 py-3 text-right">
                  <Link href={`/admin/services/${row.id}/edit`} className="mr-2 inline-flex rounded-lg p-2 text-[#7B6EE8] hover:bg-white/5">
